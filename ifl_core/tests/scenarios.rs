...
fn test_scenario_summarize_paste() {
    ...
    for ch in "Check this out:".chars() {
        core.push_event(&id, InputEvent::KeyInsert { ch, ts }).unwrap();
        ts += 100;
    }

    // Simulate pasting a large block
    core.push_event(&id, InputEvent::Paste { length: 500, ts }).unwrap();
    ts += 500;
...
fn test_scenario_refine_typing() {
    ...
    for _ in 0..50 {
        core.push_event(&id, InputEvent::KeyInsert { ch: 'a', ts }).unwrap();
        ts += 1000; // Slow typing, total 50s
    }

    // Backspace a lot
    for _ in 0..25 {
        core.push_event(&id, InputEvent::KeyDelete { 
