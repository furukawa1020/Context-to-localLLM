...
impl StructureAnalyzer {
    pub fn analyze(text: &str) -> StructureFeatures {
        ...
        let question_like = text.trim().ends_with('?') || text.contains('?') || text.trim().ends_with('？');
        
        let command_like = {
            let lower = text.to_lowercase();
            lower.starts_with("please") || lower.starts_with("write") || lower.starts_with("create") ||
            text.contains("して") || text.contains("ください")
        };
        ...
        let request_summary = text.contains("summarize") || text.contains("要約") || text.contains("まとめて");
